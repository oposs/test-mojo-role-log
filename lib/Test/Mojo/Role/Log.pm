...
sub log_trace_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'trace','like',$desc);
}

sub log_debug_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'debug','like',$desc);
}

sub log_info_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'info','like',$desc);
}

sub log_warn_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'warn','like',$desc);
}

sub log_error_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'error','like',$desc);
}

sub log_fatal_like ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'fatal','like',$desc);
}

sub log_trace_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'trace','unlike',$desc);
}

sub log_debug_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'debug','unlike',$desc);
}

sub log_info_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'info','unlike',$desc);
}

sub log_warn_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'warn','unlike',$desc);
}

sub log_error_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'error','unlike',$desc);
}

sub log_fatal_unlike ($self,$rx,$desc=undef) {
    return $self->_log_test($rx,'fatal','unlike',$desc);
}
